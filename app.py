...
<style>
:root{
  --text:#f8fafc; --muted:#a8b4c7; --green:#3ee17b; --red:#ff6b6b;
}
...
@media (max-width: 900px){
  ...
  .card .value{ font-size:1.06rem; }
  .card .value.big{ font-size:1.42rem; }
  .card .sub{ font-size:.94rem; }
...
def get_fx_card_data():
    mapping = {"달러":"KRW=X","위안":"CNYKRW=X","엔":"JPYKRW=X","유로":"EURKRW=X"}
    out = {}
...
def get_base_rate():
    ...
    if value is not None:
        return {"ok": True, "value": value, "message": "직전 변경 대비 정보는 별도 미연결"}
...
def get_gold_kr():
    ...
    try:
        r = requests.get("https://m.koreagoldx.co.kr/", headers=HEADERS, timeout=8)
        compact = re.sub(r"\s+", " ", r.text)
        m = re.search(r"순금시세\s*Gold24k-3\.75g\s*([0-9,]+)\s*.*?\s([0-9,]+)\s*(?:-?[0-9,.%a-zA-Z ]*)", compact)
        if m:
    ...
    try:
        if sell is None:
            r = requests.get("https://www.exgold.co.kr/", headers=HEADERS, timeout=8)
            compact = re.sub(r"\s+", " ", r.text)
            m = re.search(r"사업자 매입 시세\s*;?\s*순금시세,\s*([0-9,]+)원", compact)
            if m:
                sell = int(m.group(1).replace(",", ""))
        if buy is None:
            r = requests.get("https://www.exgold.co.kr/price/inquiry/domestic", headers=HEADERS, timeout=8)
            compact = re.sub(r"\s+", " ", r.text)
            m = re.search(r"금기준가:\s*[0-9,\.]+\(₩/g\):\s*([0-9,]+)\(₩/3\.75g\)", compact)
            if m:
                base_375 = int(m.group(1).replace(",", ""))
                buy = int(round(base_375 * 1.17))
                note = "금기준가 기반 추정 판매가"
    except Exception:
        pass
...
def make_stock_table(items):
    ...
    for name, ticker in items:
        row = yf_last_two(ticker)
        if row:
            rows.append({"종목": name, "티커": ticker, "현재가": fmt_int(row["price"]), "전일대비": f"{row['diff']:+,.0f}", "등락률(%)": f"{row['pct']:+.2f}"})
        else:
...
r1 = st.columns(4)
with r1[0]:
    render_card("오늘의 코스피", fmt_num(kospi["price"]) if kospi else "-", delta_html(kospi["diff"], kospi["pct"]) if kospi else "전일 대비 정보 없음", "Yahoo Finance")
with r1[1]:
    render_card("오늘의 코스닥", fmt_num(kosdaq["price"]) if kosdaq else "-", delta_html(kosdaq["diff"], kosdaq["pct"]) if kosdaq else "전일 대비 정보 없음", "Yahoo Finance")
with r1[2]:
    render_card("한국 금시세 1돈 · 살때", f"₩{fmt_int(gold.get('buy'))}" if gold.get("buy") else "-", "전일 대비 정보 없음", "한국금거래소 공개 페이지" if gold.get("ok") else None, gold.get("message"), big=True)
with r1[3]:
    render_card("한국 금시세 1돈 · 팔때", f"₩{fmt_int(gold.get('sell'))}" if gold.get("sell") else "-", "전일 대비 정보 없음", "한국금거래소 공개 페이지" if gold.get("ok") else None, gold.get("message"), big=True)

r2 = st.columns(4)
with r2[0]:
    if base_rate.get("ok"):
        render_card("한국 기준금리", f"{base_rate['value']:.2f}%", base_rate["message"], "한국은행", big=True)
    else:
        render_card("한국 기준금리", "-", base_rate.get("message", "직전 변경 대비 정보 없음"), None, None, big=True)
with r2[1]:
    if fx:
        parts = []
        ...
        first = fx.get("달러")
        render_card("원화환율", "<br>".join(parts), delta_html(first["diff"], first["pct"], unit="원", prefix="달러 기준") if first else "달러 기준 정보 없음", "Yahoo Finance", big=False)
    else:
        render_card("원화환율", "-", "환율 데이터 없음", big=False)
with r2[2]:
    render_card("국제유가 · 브렌트유", f"${fmt_num(brent['price'])} / bbl" if brent else "-", delta_html(brent["diff"], brent["pct"], unit=" 달러") if brent else "전일 대비 정보 없음", "Yahoo Finance")
with r2[3]:
    if opinet.get("ok"):
        notes = []
        if opinet.get("gas_diff") is not None: notes.append(f"휘발유 {opinet['gas_diff']:+,.0f}원")
        if opinet.get("diesel_diff") is not None: notes.append(f"경유 {opinet['diesel_diff']:+,.0f}원")
        render_card("한국 기준 유가", f"휘발유 {fmt_num(opinet.get('gas'),0)}원<br>경유 {fmt_num(opinet.get('diesel'),0)}원", "전일 대비 " + (" · ".join(notes) if notes else "정보 없음"), "오피넷", big=False)
    else:
        render_card("한국 기준 유가", "API 키 확인 필요", opinet.get("message", "오피넷 데이터 없음"), "오피넷", big=False)
...
with right:
    ...
    <div class="link-list">
      ...
      <a href="https://www.bok.or.kr/portal/main/main.do" target="_blank">한국은행 기준금리</a>
